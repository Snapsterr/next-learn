...
const SearchField = () => {
  ...
  const closeSearchResult = (event: Event): void => {
    setIsSearchResultOpen(false)
  }

  useOnClickOutside(searchRef, closeSearchResult)

  return (
    <div className="w-fit h-fit relative flex flex-col" ref={searchRef}>
      <div>
        <button
          className="btnSearch w-8 h-8 px-1 border-none outline-none cursor-pointer absolute right-0 bg-transparent hover:text-[#b3b3b3]"
        ...
        <input
          ...
          className={` h-8 w-8 border-b-[1px] p-[16px] border-b-transparent text-base bg-transparent tracking-wider outline-0 transition-all duration-500 ease-in-out text-[#fff] text-[14px] placeholder:text-[#fff]/50 placeholder:text-[14px] placeholder:tracking-wider placeholder:font-light focus:w-[200px] focus:pl-[6px] focus:rounded-none focus:bg-transparent focus:border-b-[1px] focus:border-b-[#fff]/50 max-lg:focus:w-[140px] ${
            isSearchResultOpen
              ? "w-[200px] pl-[6px] rounded-none bg-transparent border-b-[1px] border-b-[#fff]/50 transition-all duration-500 ease-search-in-out max-lg:w-[140px]"
              : ""
          }`}
